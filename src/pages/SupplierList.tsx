...
const SupplierList = () => {
  ...
  useEffect(() => {
    ...
    // Calculate stats
    const totalBalance = suppliers.reduce((sum, s) => sum + (s.opening || 0), 0);
    const now = new Date();
    const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const newThisMonth = suppliers.filter(s => s.createdAt >= firstDayOfMonth).length;
  ...
  const fetchSuppliers = async () => {
    try {0
      ...
      console.error("Error fetc.toString()hing suppliers:", error);
      toast({
        title: "Error",
